...
> - ✅ **Phase 4 applied (redesigned)** — after seeing the full code, a literal `Bus::batch` conversion was rejected: it could only replace the 80% trigger + dispatch guard (the `segments_ready` Lua counter feeds the UI/readiness and completion stays with `GenerateBgChunkJob`), at the cost of a migration + `Batchable` plumbing + MySQL writes per job. Instead: **one wave-dispatch path** (`app/Services/WaveDispatcher.php`, called in-process by Prepare for wave 0 and by a now-thin `DispatchWaveJob` for waves 1+), **SET NX claim keys** replacing the `wavesDispatchedKey` counter-CAS, and the wave **offset folded into the payload** (kills the `:offset` sibling key). Same pacing (micro-batch fast start, 15s-staggered lookahead, 80% waterfall), −276/+93 lines, deploy-safe for in-flight sessions (legacy payload + constructor compat). Verified with a Queue::fake behavioral suite: 6 dispatch scenarios + Prepare end-to-end + exactly-once waterfall claim.
