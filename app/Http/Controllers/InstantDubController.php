...
use App\Services\HlsMasterRewriter;
...
class InstantDubController extends Controller
{
    ...
    public function stop(string $sessionId): JsonResponse
    {
        ...
        $this->deleteDirectory(sys_get_temp_dir() . '/instant-dub-web/' . $sessionId);
    ...
    public function hlsMaster(string $sessionId)
    {
        ...
        $result = (new HlsMasterRewriter())->rewrite(
            $master,
            $videoUrl,
            $session['video_base_url'] ?? '',
            $session['video_query'] ?? '',
            $session['language'] ?? 'uz',
            $dubPlayable,
        );
    ...
    public function hlsAudioPlaylist(string $sessionId)
    {
        ...
        foreach ($bgHashData as $bgIdx => $bgJson) {
            ...
            if ($cs < $dubStartTime) {
                ...
                $sliceFile = $this->webCacheDir($sessionId) . "/bg-{$bgIdx}-from-{$offsetMs}.ts";
    ...
    public function hlsLeadSegment(string $sessionId)
    {
        ...
        if ($dubStartTime > 0.25) {
            ...
            if ($this->generateLeadFromHls($sessionId, $duration)) {
                return response()->file($this->webCacheDir($sessionId) . '/lead.ts', [
    ...
    public function hlsBgSegment(string $sessionId, int $index)
    {
        ...
        if (InstantDubHlsReadiness::chunkHasVerifiedDub($sessionId, $session, $index, $bgChunk, $aacDir)) {
            return $this->shiftedTsResponse($sessionId, $aacFile, $cs, $dur);
        }
    ...
    public function hlsBgSourceSegment(string $sessionId, int $index)
    {
        ...
        $rawFile = $bgChunk['path'] ?? null;
        if ($rawFile && file_exists($rawFile) && filesize($rawFile) > 10) {
            return $this->shiftedTsResponse($sessionId, $rawFile, $cs, $dur);
        }
    ...
    public function hlsBgSliceSegment(string $sessionId, int $index, int $offsetMs)
    {
        ...
        $sliceFile = $this->webCacheDir($sessionId) . "/bg-{$index}-from-{$offsetMs}.ts";
        ...
        return $this->shiftedTsResponse($sessionId, $sliceFile, $cs + $offset, $duration);
    }
     ...
     */
    /**
     * Session-scoped scratch dir for artifacts generated by web requests.
     * Storage dirs are created by Horizon workers running as root, so
     * php-fpm (www-data) cannot write next to the chunk files.
     */
    private function webCacheDir(string $sessionId): string
    {
        $dir = sys_get_temp_dir() . "/instant-dub-web/{$sessionId}";
        if (!is_dir($dir)) {
            @mkdir($dir, 0777, true);
        }
    ...
    private function shiftedTsResponse(string $sessionId, string $file, float $absStart, float $fallbackDuration)
    {
        ...
        $shifted = $this->webCacheDir($sessionId) . '/' . preg_replace('/\.ts$/', ".pts{$offsetMs}.ts", basename($file));
    ...
    private function generateLeadFromHls(string $sessionId, float $duration): bool
    {
        ...
        // Written from a web request — storage dirs are root-owned (Horizon),
        // so the web-generated lead lives in the web cache dir.
        $cacheDir = $this->webCacheDir($sessionId);
        $leadFile = "{$cacheDir}/lead.ts";

        // File lock — only one request generates; others fall through to silence
        $lockFile = "{$cacheDir}/lead.lock";
        ...
        try {
            ...
            foreach ($audioLines[0] ?? [] as $line) {
                if (preg_match('/URI="([^"]+)"/', $line, $m)) {
                    return (new HlsMasterRewriter())->resolveHlsUrl($baseUrl, $m[1], $query);
                }
            }
        } catch (\Throwable) {}
        return null;
