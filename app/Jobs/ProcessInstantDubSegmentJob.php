...
class ProcessInstantDubSegmentJob implements ShouldQueue
{
    ...
    private function generateGapAac(): void
    {
        ...
        try {
            Process::timeout(15)->run([
                ...
                '-i', 'anullsrc=r=44100:cl=mono',
                '-c:a', 'aac', '-b:a', '32k', '-f', 'mp4', '-movflags', '+frag_keyframe+empty_moov+default_base_moof', $gapFile,
            ]);
        } catch (\Throwable $e) {
        ...
        try {
            if ($hasBg) {
                ...
                Process::timeout(20)->run([
                    ...
                    '-ac', '1', '-c:a', 'aac', '-b:a', '64k', '-f', 'mp4', '-movflags', '+frag_keyframe+empty_moov+default_base_moof', $gapFile,
                ]);
            } else {
                Process::timeout(15)->run([
                    'ffmpeg', '-y', '-f', 'lavfi', '-t', (string) $gapDuration,
    ...
    private function incrementReady(): void
    {
        ...
        $lua = <<<'LUA'
            ...
            if session['segments_ready'] >= total and hasBg then
