...
class TranslateInstantDubMicroBatchJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // Merge voice map for these speakers (strip emotion tag)
        $speakers = [];
        foreach ($translated as $seg) {
            $speakers[explode(':', $seg['speaker'] ?? 'M1', 2)[0]] = true;
        }
    ...
    private function translateMicroBatch(array $segments, string $fullDialogue): array
    {
        ...
        $systemPrompt = "You are a fluent bilingual film dubbing translator. UNDERSTAND the original dialogue — its meaning, subtext, humor, emotions — then EXPRESS it in natural spoken {$toLang} as if the film were originally in {$toLang}. "
            ...
            . 'Example: "1. [M1:angry] Get out!" or "2. [F1:sad] I never wanted this..."';
